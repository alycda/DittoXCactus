# Determinism Harness — U1 / R2 pre-flight gate

Standalone Flutter package for the Mesh RAG demo's **iOS↔Android embedding
determinism spike** (Implementation Unit U1, Requirement R2). It exists to
answer one question before the rest of the build is locked:

> Does the chosen Cactus slug pair produce stable enough embeddings across iOS
> and Android that top-k retrieval ordering agrees on ≥95% of a 20-query
> fixture?
...
├── run.dart                           # CHECK MODE: Dart CLI that loads two
│                                      # per-device outputs and computes the
│                                      # agreement rate (exit 0 / 1 / 2)
├── test/
...
| `1`  | `agreement_rate < 0.95` — **R2 gate fails**. Cut-order pivot to option C. Disagreeing queries are dumped to stdout. |
| `2`  | Usage error (missing args, file not found, malformed JSON). |

When the rate lands in the 0.85–0.95 diagnostic band, the CLI prints a
follow-up note suggesting kernel-pin tightening (batch invariance, quant,
backend lock — see the
...
29 tests cover the math layer (cosine, top-k tie-break, agreement_rate
helpers, fixture parsing, JSON round-trip) and an end-to-end pass with
synthetic embeddings. None of these depend on Cactus — they ensure the
agreement-rate calculation itself is never where bugs hide. The on-device
sanity gate (a "≥80% of fixture queries put expectedTop1 in top-3" floor)
lives in the integration test and only fires against real Cactus output.
...
This harness only answers the embedding-determinism question. It is run
**once** before the loop and **again** in U13 with the locked slug pair to
produce the checked-in baseline.
