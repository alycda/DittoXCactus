...
- **Small specialist / domain-expert models (future-work angle).** For Stage 0 we ship a generalist. The writeup gestures at the next move: a mesh of small *specialist* models (a sous-chef LLM, a maintenance-manual LLM, a triage LLM) composed across devices. Collect the strongest prior art on small specialist / fine-tuned domain-expert LLMs: **LoRA / QLoRA** at small scale, **DistilBERT-style** task-specific distillation, **MoE small-scale** experiments, **TinyLlama / SmolLM specialist forks**, **AI agents specialized via DPO**, and any published recipes for "train a 1B specialist that beats a 7B generalist on a narrow task." Cite primary results, not vibes. Mark this section as feeding the writeup, not the Stage-0 build.
