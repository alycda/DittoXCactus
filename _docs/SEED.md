...
1. **Corpus theme for Stage 0 (constrained: disjoint-by-design pairs with visible note-ID citations).** Generic notes (movies / study notes / travel), Ditto-canonical `cars`-collection vibe, or a niche that makes the demo memorable? The theme must fit within ~50 notes/device and must support the disjoint-by-design pair construction Holdout 1 requires. Bind to whichever Q4 framing wins — a privacy framing wants a sensitive-notes corpus; a "knowledge composes when devices meet" framing wants complementary-knowledge pairs.
...
4. **Durability of the thesis — RESOLVE BEFORE LOOP STARTS.** The writeup needs a non-rising-tide answer to "why does this still matter when cloud RAG is cheaper and faster in a year?" Candidate framings to stress-test: (i) data sovereignty / consent-scoped sharing (re-framed from "privacy" after the Why This Exists scoping); (ii) offline-by-default; (iii) "knowledge composes when devices meet, like Bluetooth pairing for ideas"; (iv) opportunistic knowledge composition in bandwidth-denied environments — disaster zones, ships, transit, classified facilities. Per Apply More Tokens, write 250 words of each and stress-test against the strongest skeptic before locking. Q1 corpus choice depends on the outcome.
