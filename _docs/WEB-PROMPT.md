# Deep-Research Prompt — Mesh RAG (Ditto × Cactus)
...
**Important context for this run:** three sibling research passes (Claude Agent, OpenAI Codex CLI, Google Gemini CLI) have already executed the same brief using their built-in web search. Your value as the fourth pass is to:

1. **Go deeper.** You have more time and a longer citation chain. Find the primary sources the CLI agents skimmed over — the actual arxiv PDFs, GitHub commits, the specific docs pages, the original conference talks (with video links).
2. **Hunt for gaps.** The four areas where CLI agents tend to underperform: (a) papers from the last 18 months on niche topics, (b) engineering blog posts from teams that built the thing (vs. summarizers), (c) GitHub repos with low SEO but high signal, (d) conference talks with video that haven't been transcribed.
...
Mark anything you find that you suspect would not surface in a CLI agent's first pass.
...
The concrete corpus CRDT shape:
...
- Local-first software essay (Kleppmann, Beyer, McGranaghan, van Hardenberg — Ink & Switch, 2019).
...
- **Structured-list merge / aggregation quality at small parameter counts.** The candidate demo corpus is recipes (audience-submitted variants of the same dish, e.g., chicken tortilla soup) that the on-device LLM must normalize / synthesize into a coherent merged recipe after Ditto sync. Find benchmarks or anecdotal reports on small LLMs (≤ 3B) doing structured-list reconciliation: ingredient merging, deduplication, quantity normalization. Specifically: at what parameter count does this task stop being terrible? Is there a published eval (e.g., SummEval-style or list-aggregation-specific) we can use to pre-screen models?
- **Small specialist / domain-expert models (future-work angle).** Collect the strongest prior art on small specialist / fine-tuned domain-expert LLMs: **LoRA / QLoRA** at small scale, **DistilBERT-style** task-specific distillation, **MoE small-scale** experiments, **TinyLlama / SmolLM specialist forks**, **AI agents specialized via DPO**, and any published recipes for "train a 1B specialist that beats a 7B generalist on a narrow task." Cite primary results, not vibes.
...
- **Cactus + Ditto are the chosen stack.** Find prior art that helps us *succeed with them*, plus enough adjacent comparisons to credibly defend the choices. Do not propose swapping them.

## Scope guardrails (don't bother)

- Don't research general transformer architectures or LLM training. Off-the-shelf models only.
